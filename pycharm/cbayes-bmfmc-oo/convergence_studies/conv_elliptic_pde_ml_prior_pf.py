...
if __name__ == '__main__':
    ...
    n_evals_mc = np.logspace(np.log10(30), np.log10(1000), 10)
    n_evals_mc = np.round(n_evals_mc).astype(int)
    n_evals_bmfmc_hf = np.logspace(np.log10(5), np.log10(500), 10)
    n_evals_bmfmc_hf = np.round(n_evals_bmfmc_hf).astype(int)
    n_evals_bmfmc_mf = np.logspace(np.log10(10), np.log10(500), 10)
    ...
    total_costs_1lf = n_evals_bmfmc_mf
    ...
    for idx, n_evals in enumerate(n_evals_bmfmc_mf):
